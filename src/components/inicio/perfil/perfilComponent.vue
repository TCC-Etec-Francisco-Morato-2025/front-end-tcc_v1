...
import popUpInfoItem from './popUpInfoItem.vue';
...
const mudarNome = async () => {
  ...
  if (newNome.value) await userStore.mudarNome(newNome.value);
};

// mudar imagem
const enviarImg = async () => {
  if (imgItem.value) await userStore.mudarImg(imgItem.value);
  ...
  <q-dialog v-model="popUpStore.perfil" :maximized="popUpStore.perfil">
    <q-card>
      <q-card-actions align="right">
        <q-btn icon="close" class="absolute-top-right q-ma-lg z-top" style="color: rgb(var(--cor-principal-1));" size="20px" round @click="popUpStore.togglePerfil()" flat/>
      </q-card-actions>
      <q-card-section align="center">
        ...
        <q-dialog v-model="editImage">
          <q-card style="width: 300px">
            <q-btn icon="close" class="absolute-top-right q-pa-md z-top" @click="editImage = false" dense flat rounded />
            <q-card-section align="center">
              <q-file
                ref="inputImg"
                v-model="imgItem"
            ...
            <q-card-actions align="right">
              <q-btn label="Cancelar" color="red" dense no-caps @click="editImage = false" />
      ...
      <q-card-actions class="box-itens center" align="left" vertical>
        ...
        <div class="nome-itens atac">Itens de Ataque</div>
        <div class="box-item center">
          <q-btn
            ...
            push
          >
            <pop-up-info-item :item="item" />
          </q-btn>
        </div>
        <div class="nome-itens def">Itens de Defesa</div>
        <div class="box-item center">
          <q-btn
            class="itens"
        ...
        <div class="nome-itens espec">Itens Especiais</div>
        <div class="box-item center">
          <q-btn
            class="itens"
...
.nome-itens.espec {
  background-color: rgb(var(--cor-principal-2));
}
