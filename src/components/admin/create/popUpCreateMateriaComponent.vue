...
const corMateria = ref('#ffffff');
const corLetra = ref('#000000');
const path=ref('');
...
const criar = (nome:string, icon:string, corMateria: string, corLetra:string)=>{
  path.value=nome.toLocaleLowerCase().trim().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
  ...
  console.info('Nova matéria criada:', newMateria);
}
...
<template>
  <q-dialog v-model="popUpStore.createMateriaPopUp" :maximized="popUpStore.createMateriaPopUp">
    <div class="popUp">
      ...
      <div class="box-carac center">
        ...
        <q-input label="Cor da Matéria:" v-model="corMateria" class="my-input" outlined>
          <template v-slot:append>
            <div class="selec-cor" :style="'background-color: ' + corMateria">
              <q-popup-proxy cover transition-show="scale" transition-hide="scale">
                <q-color v-model="corMateria" />
              </q-popup-proxy>
            </div>
          </template>
        </q-input>

        <q-input label="Cor da Letra:" v-model="corLetra" class="my-input" outlined>
          <template v-slot:append>
            <div class="selec-cor" :style="'background-color: ' + corLetra">
              <q-popup-proxy cover transition-show="scale" transition-hide="scale">
                <q-color v-model="corLetra" />
              </q-popup-proxy>
            </div>
          </template>
        </q-input>
        ...
        <div class="btn">
          ...
          <q-btn label="cancelar" color="red" @click="popUpStore.toggleCreateMateriaPopUp()" push rounded />
        </div>
      </div>
    </div>
  </q-dialog>
...
.selec-cor{
  /* background-color: v-bind(corMateria) !important; */
  width: 20px;
  height: 20px;
  cursor: pointer;
}
